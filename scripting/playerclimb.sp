...
new Handle:cClimbScout, Handle:cClimbSoldier, Handle:cClimbPyro, Handle:cClimbDemo, Handle:cClimbHeavy, Handle:cClimbEngie, Handle:cClimbMedic, Handle:cClimbSniper, Handle:cClimbSpy, Handle:cClimbCivilian;
new maxClimbs[MAXPLAYERS+1] = {0, ...};
new bool:gClimb[MAXPLAYERS+1][10];
...
public OnPluginStart()
{
	...
	cvarClass = CreateConVar("sm_playerclimb_class", "all", "Which classes should be allowed to climb? You can add multiple classes by separating them with a comma (EX: scout,sniper,spy,heavy,soldier,demo,medic,pyro,engineer,civilian). For all classes, just put \"all\" (no quotes).");
	...
	cClimbCivilian =		RegClientCookie("sm_playerclimb_cookie_civilian", "", CookieAccess_Private);
	
	AutoExecConfig(true, "PlayerClimb");
	
	for (new i = 1; i <= MaxClients; i++)
	{
		for (new col = 0; col < 10; col++)
		{
			gClimb[i][col] = true;
		}
...
CreatePlayerClimbMenu(client)
{
	...
	if ((StrContains(cvClass, "civilian", false) != -1) || allClasses)
	{
		if (gClimb[client][9])
			AddMenuItem(menu, "civilian_false", "[x] Civilian");
		else
			AddMenuItem(menu, "civilian_true", "[ ] Civilian");
	}
...
public CreatePlayerClimbMenuCallback(Handle:menu, MenuAction:action, client, param2)
{
	...
	if (action == MenuAction_Select)
	{
		...
		if (StrEqual(info, "civilian_true"))	{		gClimb[client][9] = true;		SetClientCookie(client, cClimbCivilian, info);		}
		if (StrEqual(info, "civilian_false")) {		gClimb[client][9] = false;		SetClientCookie(client, cClimbCivilian, info);		}
...
public OnClientCookiesCached(client)
{
	...
	GetClientCookie(client, cClimbCivilian, info, sizeof(info));
	if (StrEqual(info, "civlian_true"))			gClimb[client][9] = true;
	if (StrEqual(info, "civlian_false"))		gClimb[client][9] = false;
}
...
public Action:TF2_CalcIsAttackCritical(client, weapon, String:weaponname[], &bool:result)
{
	...
	switch (TF2_GetPlayerClass(client))
	{
		...
		case TFClass_Civilian:		if (!gClimb[client][9]) return Plugin_Continue;
	}
...
stock bool:IsClassAllowed(client)
{
	...
	switch (TF2_GetPlayerClass(client))
	{
		...
		case TFClass_Civilian:	if (StrContains(cvClass, "civilian", false) != -1) return true;
	}
	return false;
}
