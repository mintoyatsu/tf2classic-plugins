...
public Plugin:myinfo = {
	name = "[TF2C] Scout Nail Jump",
	author = "mintoyatsu",
	description = "Jump with nail gun",
...
{
	sm_needlejump_version = CreateConVar("sm_nailjump_version", PLUGIN_VERSION, "TF2C Scout Nail Jump plugin version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED);
	...
	sm_needlejump_enabled = CreateConVar("sm_nailjump_enabled", "1", "", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	HookConVarChange(sm_needlejump_enabled, OnConVarChanged);
	
	sm_needlejump_prethink = CreateConVar("sm_nailjump_prethink", "0", "Use OnPreThink instead of OnGameFrame?", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public OnPreThink( iClient )
{
	...
	if( !StrEqual( strClassname, "tf_weapon_nailgun", false ) )
		return;
