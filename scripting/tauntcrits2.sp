...
public OnPluginStart()
{
	...
	cvarCivilian = CreateConVar("tauntcrits_civilian","2.0","Award duration(after finishing the taunt) for civilian",_,true,0.00,true,360.00);
	...
	HookConVarChange(cvarCivilian, CvarChange);
...
public Action:help(client, args)
{
	...
	PrintToConsole(client, "     Civilian:%f",g_Civilian);
...
public CvarChange(Handle:convar, const String:oldValue[], const String:newValue[]){
	...
	if (convar==cvarCivilian)
		g_Civilian=StringToFloat(newValue);
...
readCvars()
{
	...
	g_Civilian=GetConVarFloat(cvarCivilian);
